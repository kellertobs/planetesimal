function [Div_va] = phi_adv(U,W,a,dx,dz,scheme)
...
switch scheme
    ...
    case 'centred FD' % not recommended
        ...
        Div_va = vx.*(aip-aim)./h + vz.*(ajp-ajm)./h;
                
    case 'flxdiv'
        
        Div_va = ((ajp+acc)./2.*wp - (ajm+acc)./2.*wm)./dz ...
             + ((aip+acc)./2.*up - (aim+acc)./2.*um)./dx;             
                
    case 'fromm'
        
        Div_va   =     up .*(-(aipp-aip)./dx./8 + (aip + acc)./dx./2 + (acc-aim )./dx./8) ...
