...
zeta = MAT.Eta.s(2:end-1,2:end-1)./max(SOL.philim,SOL.phi(2:end-1,2:end-1));
% segregation coefficient
KD = MAT.kU(2:end-1,2:end-1)./MAT.EtaU.l(2:end-1,2:end-1);
...
% right boundary
ii  = indW(NUM.flag.W(:,end)   == 0); jj1 = ii;
jj2 = indW(NUM.flag.W(:,end-1) == 0);
aa = zeros(size(ii));
II = [II; ii(:)]; JJ = [JJ; jj1(:)];   AA = [AA; aa(:)+1];
...
% top & bottom
%               top             ||          bottom
ii = [indW(NUM.flag.W(1,:) == 0); indW(NUM.flag.W(end,:) == 0)]; jj = ii;
aa = zeros(size(ii));
...
% X Stokes equation
% non-boundary-conforming Dirichlets
% top boundary
% ii = indU(1,:).'; jj1 = ii; jj2 = indU(2,:).';
ii  = indU(NUM.flag.U(1,:) ==0).'; jj1 = ii; 
jj2 = indU(NUM.flag.U(2,:) ==0).';
aa = zeros(size(ii));
II = [II; ii(:)]; JJ = [JJ; jj1(:)];   AA = [AA; aa+1];
...
% bottom boundary
ii  = indU(NUM.flag.U(end,:) ==0).'; jj1 = ii; 
jj2 = indU(NUM.flag.U(end-1,:) ==0).';
aa  = zeros(size(ii));
II  = [II; ii(:)]; JJ = [JJ; jj1(:)];   AA = [AA; aa(:)+1];
II  = [II; ii(:)]; JJ = [JJ; jj2(:)];   AA = [AA; aa(:)+SOL.BCbot];
IR  = [IR; ii(:)]; RR = [RR; aa(:)];

% left & right side boundary
%               right            ||             left
ii = [indU(NUM.flag.U(:,1) == 0); indU(NUM.flag.U(:,end) == 0)]; jj = ii;
aa = zeros(size(ii));
II = [II; ii(:)]; JJ = [JJ; jj(:)];   AA = [AA; aa+1];
IR = [IR; ii(:)]; RR = [RR; aa(:)];
...
% internal points (which are active cells)
...
% check flag surrounding nodes (void nodes multiply by zero, active nodes multply by 1)
%               left                     ||                 right  
fl  = NUM.flag.U(3:end,2:end-1)     == 0;   fr  = NUM.flag.U(2:end-1,3:end) == 0;
ft  = NUM.flag.U(1:end-2,2:end-1)   == 0;   fb  = NUM.flag.U(3:end,2:end-1) == 0;
fi  = NUM.flag.U(2:end-1,2:end-1)   == 0; % current node

aa = -2/3*(EtaP1+EtaP2)./NUM.dx^2 - 1/2*(EtaC1+EtaC2)./NUM.dz^2;
II = [II; ii(:)]; JJ = [JJ;  ii(:)];   AA = [AA; fi.*aa(:)             ];          % U on stencil centre
II = [II; ii(:)]; JJ = [JJ; jj1(:)];   AA = [AA; fl.*2/3*EtaP1(:)./NUM.dx^2];      % U one to the left
II = [II; ii(:)]; JJ = [JJ; jj2(:)];   AA = [AA; fr.*2/3*EtaP2(:)./NUM.dx^2];      % U one to the right
II = [II; ii(:)]; JJ = [JJ; jj3(:)];   AA = [AA; ft.*1/2*EtaC1(:)./NUM.dz^2];      % U one above
II = [II; ii(:)]; JJ = [JJ; jj4(:)];   AA = [AA; fb.*1/2*EtaC2(:)./NUM.dz^2];      % U one below
...
% internal points
ii = indU(2:end-1,2:end-1);
aa = -1./KD; % inverse of segregation coefficient
II = [II; ii(:)]; JJ = [JJ;  ii(:)];   AA = [AA; aa(:)];      % W on stencil centre
...
% free surface Dirichlet boundary contition (P = 0)
