...
%% setup free surface chain y = Fs(x)
% NUM.Fs = FreeSurface(NUM.xC,NUM.D/10);
NUM.Fs = NUM.D/10;
% Set grid flagging
NUM.flag.P = zeros(NUM.nzP,NUM.nxP);
% activate fluid cells
[zf,xf] = find(NUM.ZP>NUM.Fs);
NUM.flag.P(zf,xf) = 1; 
% activate free surface cells (zero pressure) above the free surface
NUM.flag.P(min(zf)-1,:) = 2;
% [zf,xf] = find(NUM.flag.P

% flag vx nodes
NUM.flag.U = zeros(NUM.nzU,NUM.nxU);
NUM.flag.U = NUM.flag.P(:,1:end-1).*NUM.flag.P(:,2:end); % 0 = void, 1 = active, 2 = free surface, 4 = between free surface/do not solve
NUM.flag.U(NUM.flag.U>3) = 0;
