...
M           = 5;   % number of time steps to take
...
%     M = M0*h0./h;
    ...
    % while max([max(afr(:))-min(afr(:)),max(au3(:))-min(au3(:))]) < 2*a1 && ti <= M
    for tt= 1:1:M
        % while ti <= M
...
hold on
CFL_ref = [1,0.1];
% ref = max(max(resend_afr(:),resend_au3(:)));
ref = 0.001;
lin = [ref,ref*0.1^1];
qdr = [ref,ref*0.1^2];
cub = [ref,ref*0.1^3];
loglog(CFL_ref,lin,'k--',CFL_ref,qdr,'b--',CFL_ref,cub,'r--');
legend('Central difference','flux divergent','fromm','first upwind','second upwind','third upwind')
