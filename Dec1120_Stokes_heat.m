...
%     time = time+dt;
    ...
    xvz,zvz,xp,zp,Tm,rhom,etam,km,rhocpm);
    ...
    dT = T2-T1;
    %solve temperature markers
    %temperature on subgrid
    dTsub = Tempcorrection(dsubgridt,nx1,nz1,nx,nz,marknum,xm,zm,xp,zp,...
    dx,dz,rhocpm,km,matm,T1,Tm,dt);
    %corrected dT
    dT = dT - dTsub;

    %update T markers
    Tm = UpdateTm(Tm,dT,marknum,xm,zm,xp,zp,dx,dz,nx,nz,T2,ti);
...
function [Ha,Hs] = shearheating(Ha,Hs,nx,nz,dx,dz,Epsxz,Epsxx,Sigxz,Sigxx,...
    vx_out,vz_out,Eta_out,Eta_mid,Rho_vz,T1,gz,Alpha)
...
function T_new = T_diffusion_implicit(T,k_vx,k_vz,CP_mid,dt,...
    dx,dz,Ha,Hs,nx1,nz1,nx,nz)
indP = numsetup(nz1,nx1);
...
A(indP(1,j),indP(2,j))      = 1;
RHS(indP(1,j))              = 273*2;
%bottom
A(indP(nz1,j),indP(nz1,j))  = 1;
A(indP(nz1,j),indP(nz,j))   = 1;
...
function dTsub = Tempcorrection(dsubgridt,nx1,nz1,nx,nz,marknum,xm,zm,xp,zp,...
    dx,dz,rhocpm,km,matm,T1,Tm,dt)
%compute dT subgrid
if(dsubgridt>0)
dTCSUM=zeros(nz1,nx1);
...
for m = 1:1:marknum
    ...
    wti1j = (1-dxm1/dx)*(dzm1/dz);
    wti1j1 = dxm1*dzm1/dx/dz;
    %calculate dT on the dubgrid
    dTmnTm = T1(i,j)*wtij + T1(i,j+1)*wtij1 + T1(i+1,j)*wti1j +...
        T1(i+1,j+1)*wti1j1 - Tm(m);
    tdiff = rhocpm(mat)/km(mat)/(2/dx^2 +2/dz^2);
    dTsubgrid = dTmnTm - dTmnTm*exp(-dsubgridt*dt/tdiff);
    % update properties
    %i,j
    dTCSUM(i,j)         = dTCSUM(i,j)       + dTsubgrid*rhocpm(mat)*wtij;
    RHOCPSUM(i,j)       = RHOCPSUM(i,j)     + rhocpm(mat)*wtij;
    %i,j+1
    dTCSUM(i,j+1)       = dTCSUM(i,j+1)     + dTsubgrid*rhocpm(mat)*wtij1;
    RHOCPSUM(i,j+1)     = RHOCPSUM(i,j+1)   + rhocpm(mat)*wtij1;
    %i+1,j
    dTCSUM(i+1,j)       = dTCSUM(i+1,j)     + dTsubgrid*rhocpm(mat)*wti1j;
    RHOCPSUM(i+1,j)     = RHOCPSUM(i+1,j)   + rhocpm(mat)*wti1j;
    %i+1,j+1
    dTCSUM(i+1,j+1)     = dTCSUM(i+1,j+1)   + dTsubgrid*rhocpm(mat)*wti1j1;
    RHOCPSUM(i+1,j+1)   = RHOCPSUM(i+1,j+1) + rhocpm(mat)*wti1j1;
end
%compute dT subgrid
dTsub = dTCSUM./RHOCPSUM;
end
end

function Tm = UpdateTm(Tm,dT,marknum,xm,zm,xp,zp,dx,dz,nx,nz,T2,timestep)
...
for m=1:1:marknum
    ...
    % Update properties
    Tm(m)=Tm(m)+dT(i,j)*wtmij+dT(i+1,j)*wtmi1j+...
            dT(i,j+1)*wtmij1+dT(i+1,j+1)*wtmi1j1;
    % Interpolate tk2 at 1st timestep
    if(timestep==1)
        Tm(m)=T2(i,j)*wtmij+T2(i+1,j)*wtmi1j+...
            T2(i,j+1)*wtmij1+T2(i+1,j+1)*wtmi1j1;
    end
end
end
